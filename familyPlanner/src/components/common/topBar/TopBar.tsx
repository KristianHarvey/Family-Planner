...
export const TopBar: React.FC<TopBarProps> = ({
		...
		if (leftIcon) {
			return (
				<Ionicons
					...
					size={FontSize.Large}
					onPress={onLeftComponentClick ? onLeftComponentClick : navigateBack}
				/>
			);
		} else if (leftText) {
			return (
	...
	const renderRightComponent = () => {
		if (rightIcon) {
			return <Ionicons name={rightIcon} color={colors.text.main} size={FontSize.Large} onPress={onRightComponentClick} />;
		} else if (rightText) {
			return (
				<Text
		...
		<View style={{ width: "100%", flexShrink: 1 }}>
			<View
				style={{
					flexDirection: "row",
					width: "100%",
					marginTop: Platform.OS === "ios" ? Padding.Small : Padding.XLarge * 3,
