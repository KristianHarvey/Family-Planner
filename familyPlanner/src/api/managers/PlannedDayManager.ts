...
import { PlannedDayInput, PlannedDayUpdate } from "../../models/plannedDay";

export class PlannedDayManager {
    public static async createNew(dayKey: string, plannedDay: PlannedDayInput) {
        const url = baseUrl + `PlannedDay/${dayKey}`;
        console.log(url);
        try {
            const response = await axiosInstance.post(url, plannedDay);
